...
(function($)
{
	...
	var methods = {
		...
		init : function(options)
		{
			...
			return this.each(function()
			{
				...
				if (showPreferences)
				{
					nav_pane_html += '<div id="' + preferences_id + '" title="' + rsc_jui_pag.preferences_title + '">' + rsc_jui_pag.preferences_text + '</div>';
				}
				...
				$.each(nav_pane_display_order, function(index,value)
				{
					...
					else if (value == 'rows_per_page')
					{
						if (showRowsPerPage)
						{
							nav_pane_html += '<input type="text" id="' + rows_per_page_id + '" ' + 'value="' + rowsPerPage + '" ' + 'title="' + rsc_jui_pag.rows_per_page_title
									+ '">';
						}
					}
				...
				if (useSlider)
				{
					...
					elem_slider.slider({ min : 1, max : totalPages, value : (sliderOrientation == 'horizontal' ? currentPage : totalPages - currentPage + 1),
						animate : sliderAnimation, range : (sliderOrientation == 'horizontal' ? 'min' : 'max'), orientation : sliderOrientation, stop : function(event,ui)
						{
				...
				if (showPreferences)
				{
					...
					elem_nav_pane.off('click', selector).on(
							'click',
							selector,
							function()
							{
								...
								elem_pref_dialog.dialog({ autoOpen : true, position : { my : "top", at : "bottom", of : '#' + container_id },
									title : rsc_jui_pag.preferences_title, buttons :
									[
										{ text : rsc_jui_pag.preferences_close, click : function()
										{
											$(this).dialog("close");
											//$(this).dialog("destroy");
										} }
					...
					elem_pref_dialog.off('click', selector).on('click', selector, function(event)
					{
						...
						elem.jui_pagination({ useSlider : state })
					});
					...
					{
						var state = $(event.target).is(":checked");
						elem.jui_pagination({ showGoToPage : state })
					});
					...
					{
						var state = $(event.target).is(":checked");
						elem.jui_pagination({ showRowsPerPage : state })
					});
					...
					{
						var state = $(event.target).is(":checked");
						elem.jui_pagination({ showRowsInfo : state })
					});
					...
					{
						var state = $(event.target).is(":checked");
						elem.jui_pagination({ showNavButtons : state })
					});
		...
		getDefaults : function()
		{
			return {
				...
				[
						'slider', 'go_to_page', 'back_buttons', 'current_page_label', 'current_page', 'nav_items', 'total_pages_label', 'total_pages', 'forward_buttons',
						'rows_per_page', 'rows_info'
				],

				useSlider : false, sliderInsidePane : true, sliderElementID : false, // if given, slider appears outside container inside specified element
				useSliderWithPagesCount : 0, // show slider over specified number of pages
				sliderOrientation : 'horizontal', sliderAnimation : 'slow', // true (or 'slow' or 'fast' or duration in milliseconds) or false

				showGoToPage : false, showNavButtons : false, showLabelCurrentPage : true, showCurrentPage : false, showNavPages : true,
				navPagesMode : 'first-last-always-visible', // alternative mode is 'continuous'
				showLabelTotalPages : false, showTotalPages : false, showRowsPerPage : false, showRowsInfo : false, showPreferences : true,

				navPaneClass : 'nav-pane ui-state-default ui-corner-all', navGoToPageClass : 'goto-page ui-state-default ui-corner-all',
				navButtonTopClass : 'nav-button-top ui-icon ui-icon-seek-first', navButtonPrevClass : 'nav-button-prev ui-icon ui-icon-seek-prev',
				navCurrentPageLabelClass : 'current-page-label', navCurrentPageClass : 'current-page', navDotsLeftClass : 'nav-dots-left', navPagesClass : 'nav-pages',
				navItemClass : 'nav-item ui-state-default ui-corner-all', navItemSelectedClass : 'nav-item ui-state-default ui-corner-all ui-state-highlight',
				navItemHoverClass : 'ui-state-hover', // if empty, no hover applied
				navDotsRightClass : 'nav-dots-right', navTotalPagesLabelClass : 'total-pages-label', navTotalPagesClass : 'total-pages',
				navButtonNextClass : 'nav-button-next  ui-icon ui-icon-seek-next', navButtonLastClass : 'nav-button-last  ui-icon ui-icon-seek-end',
				navRowsPerPageClass : 'rows-per-page  ui-state-default ui-corner-all', navInfoClass : 'rows-info', sliderClass : 'nav-slider',
				sliderInsidePaneClass : 'nav-slider-inside-pane', preferencesClass : 'preferences ui-icon ui-icon-signal',

				nav_pane_id_prefix : 'nav_pane_', nav_goto_page_id_prefix : 'goto_page_', nav_current_page_label_id_prefix : 'current_page_lbl_',
				nav_current_page_id_prefix : 'current_page_', nav_top_id_prefix : 'top_', nav_prev_id_prefix : 'prev_', nav_dots_left_id_prefix : 'dots_left_',
				nav_pages_id_prefix : 'pages_', nav_item_id_prefix : 'page_', nav_dots_right_id_prefix : 'dots_right_', nav_next_id_prefix : 'next_', nav_last_id_prefix : 'last_',
				nav_total_pages_label_id_prefix : 'total_pages_lbl_', nav_total_pages_id_prefix : 'total_pages_', nav_rows_per_page_id_prefix : 'rows_per_page_',
				nav_rows_info_id_prefix : 'rows_info_', slider_id_prefix : 'sld_', preferences_id_prefix : 'pref_', pref_dialog_id_prefix : 'pref_dlg_',
	...
	{
		return element.attr('unselectable', 'on').css('user-select', 'none').on('selectstart', false);
	};
	...
	var create_preferences = function(plugin_container_id)
	{
		...
		var a_id_ext =
		[
				'_slider', '_goto_page', '_rows_per_page', '_rows_info', '_nav_buttons'
		];
		var a_opt =
		[
				'useSlider', 'showGoToPage', 'showRowsPerPage', 'showRowsInfo', 'showNavButtons'
		];
	...
	var update_current_page = function(container_id,goto_page,update_slider)
	{
		...
		if (update_slider)
		{
			...
			if (typeof (elem_slider.data("slider")) == 'object' || typeof (elem_slider.data("uiSlider")) == 'object')
			{
				elem_slider.slider({ 'value' : (sliderOrientation == 'horizontal' ? goto_page : totalPages - goto_page + 1) });
			}
		}
